...
class NormalizadorCaja:
    ...
    def crear_carpetas(self):
        os.makedirs(f"{self.carpeta_salida}/caja", exist_ok=True)
    ...
    def limpiar_numero(self, valor):
        ...
        try:
            # 1. Detectar si el número es negativo ANTES de limpiar
            es_negativo = False
    ...
    def procesar_archivo_caja(self, archivo_path):
        nombre_archivo = os.path.basename(archivo_path)
        print(f"🔄 Leyendo archivo (Caja): {nombre_archivo}")

        # --- DETECCIÓN DE MONEDA (MEJORADA) ---
        # Separamos el nombre en palabras limpias para buscar "US" exacto sin falsos positivos
        nombre_clean = nombre_archivo.lower().replace('_', ' ').replace('.', ' ').replace('-', ' ')
        es_usd = any(x in nombre_clean.split() for x in ['us', 'usd', 'dolar', 'dólar', 'divisa', 'verde', 'extranjera'])
        ...
        try:
            ...
            for i in range(min(25, len(df_raw))): 
                ...
                if 'fecha' in fila_str and \
                   any(x in fila_str for x in ['credito', 'haber', 'debito', 'debe', 'saldo']):
                    fila_inicio = i
                    break
            ...
            col_credito = next((c for c in df.columns if any(x in c.lower() for x in ['credito', 'crédito', 'haber', 'abono'])), None)
            col_debito = next((c for c in df.columns if any(x in c.lower() for x in ['debito', 'débito', 'debe', 'cargo'])), None)
            col_motivo = next((c for c in df.columns if any(x in c.lower() for x in ['motivo', 'concepto', 'desc'])), None)
            col_partida = next((c for c in df.columns if 'partida' in c.lower()), None)
            col_registro = next((c for c in df.columns if 'registro' in c.lower()), None)
            ...
            for idx, fila in df.iterrows():
                try:
                    ...
                    motivo = str(fila.get(col_motivo, '')).strip()
                    partida = self.limpiar_referencia(fila.get(col_partida, ''))
                    registro = self.limpiar_referencia(fila.get(col_registro, ''))
                    saldo = self.limpiar_numero(fila.get(col_saldo, 0))

                    # Construir referencia con Partida y Registro
                    referencia = ''
                    if partida: referencia = partida
                    if registro: referencia = f"{referencia} - {registro}" if referencia else registro
    ...
    def guardar_archivo(self, df, nombre_base="Caja"):
        ...
        if not fechas.empty:
            ...
            path = f"{self.carpeta_salida}/caja/{nombre_base} [{rango}].xlsx"
...
def ejecutar_normalizacion(scanner_global, gestor_tasas_global):
    print(f"\n🚀 --- Iniciando Módulo: CAJA ---")
    
    normalizador = NormalizadorCaja(
        gestor_tasas=gestor_tasas_global,
        carpeta_salida="estados_cuenta_processed"
    )

    # Buscar archivos que contengan "CAJA"
    archivos_encontrados = scanner_global.listar_archivos_simples(criterio_nombre="CAJA")
    archivos_validos = [f for f in archivos_encontrados if not os.path.basename(f).startswith("~$")]

    if not archivos_validos:
        print("⚠️ No se encontraron archivos de Caja.")
        return False

    lista_dfs = []
    for ruta in archivos_validos:
        df_temp = normalizador.procesar_archivo_caja(ruta)
    ...
    if not lista_dfs: 
        print("❌ No se obtuvieron datos válidos para Caja.")
        return False
    ...
    for (mes, moneda), grupo_df in df_total_consolidado.groupby(['Mes', 'Moneda REF']):
        ...
        nombre_base = f"Caja_{sufijo_moneda}_Mes_{mes}"
...
if __name__ == "__main__":
    ...
    scanner = ScannerAutomatico(subcarpeta_datos="Ed_cuenta_Yolo_Feb")
