...
class NormalizadorBanplus:
    ...
    def crear_carpetas(self):
        os.makedirs(f"{self.carpeta_salida}/banplus", exist_ok=True)
    ...
    def procesar_archivo_banplus(self, archivo_path):
        nombre_archivo = os.path.basename(archivo_path)
        print(f"🔄 Leyendo archivo (Banplus): {nombre_archivo}")
        
        es_usd = any(x in nombre_archivo.lower() for x in ['usd', 'dolar', 'divisa', 'verde', 'panama', 'extranjera'])
        ...
        try:
            ...
            for i in range(min(25, len(df_raw))): 
                ...
                if 'fecha' in fila_str and \
                   any(x in fila_str for x in ['monto', 'cargo', 'abono', 'saldo']):
                    fila_inicio = i
                    break
            ...
            for idx, fila in df.iterrows():
                try:
                    ...
                    if es_usd:
                        ...
                        banco_nombre = 'BANPLUS_USD'    # Nombre con USD
                        tasa = 1.0
                        monto_usd = monto_bs
                    else:
                        moneda_ref = 'BS'               # Nomenclatura BS
                        banco_nombre = 'BANPLUS'        # Nombre normal
    ...
    def guardar_archivo(self, df, nombre_base="Banplus"):
        ...
        if not fechas.empty:
            ...
            path = f"{self.carpeta_salida}/banplus/{nombre_base} [{rango}].xlsx"
...
def ejecutar_normalizacion(scanner_global, gestor_tasas_global):
    print(f"\n🚀 --- Iniciando Módulo: BANPLUS ---")
    
    normalizador = NormalizadorBanplus(
        gestor_tasas=gestor_tasas_global,
        carpeta_salida="estados_cuenta_processed"
    )

    # Buscar archivos "BANPLUS"
    archivos_encontrados = scanner_global.listar_archivos_simples(criterio_nombre="BANPLUS")
    archivos_validos = [f for f in archivos_encontrados if not os.path.basename(f).startswith("~$")]

    if not archivos_validos:
        print("⚠️ No se encontraron archivos de Banplus.")
        return False

    lista_dfs = []
    for ruta in archivos_validos:
        df_temp = normalizador.procesar_archivo_banplus(ruta)
    ...
    if not lista_dfs: 
        print("❌ No se obtuvieron datos válidos para Banplus.")
        return False
    ...
    for (mes, moneda), grupo_df in df_total_consolidado.groupby(['Mes', 'Moneda REF']):
        ...
        nombre_base = f"Banplus_{sufijo_moneda}_Mes_{mes}"
