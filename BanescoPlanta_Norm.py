...
class NormalizadorBanescoPlanta:
    ...
    def crear_carpetas(self):
        os.makedirs(f"{self.carpeta_salida}/banesco_planta", exist_ok=True)
    ...
    # 🚀 Agregado: Detector inteligente de fechas
    ...
    def procesar_archivo_banesco_planta(self, archivo_path):
        nombre_archivo = os.path.basename(archivo_path)
        print(f"🔄 Leyendo archivo (Banesco Planta): {nombre_archivo}")
        ...
        try:
            ...
            for idx, fila in df.iterrows():
                try:
                    ...
                    # 🚀 Usamos el limpiador de fechas inteligente
                    ...
                    if es_usd:
                        moneda_ref = 'US'
                        banco_nombre = 'BANESCOPLANTA_USD'
                        tasa = 1.0
                        monto_usd = monto_bs
                    else:
                        moneda_ref = 'BS'
                        banco_nombre = 'BANESCOPLANTA'
    ...
    def guardar_archivo(self, df, nombre_base="BanescoPlanta"):
        ...
        if not fechas.empty:
            ...
            path = f"{self.carpeta_salida}/banesco_planta/{nombre_base} [{rango}].xlsx"
...
def ejecutar_normalizacion(scanner_global, gestor_tasas_global):
    print(f"\n🚀 --- Iniciando Módulo: BANESCO (PLANTA) ---")
    
    normalizador = NormalizadorBanescoPlanta(
    ...
    if not archivos_validos:
        print("⚠️ No se encontraron archivos de Banesco Planta.")
        return False
    ...
    for ruta in archivos_validos:
        ...
        # --- FILTRO EXCLUSIVO PARA BANESCO PLANTA ---
        if "PLANTA" not in nombre_archivo:
            print(f"   ⚠️ Saltando archivo (no es Planta): {nombre_archivo}")
            continue
        # -------------------------------------------

        df_temp = normalizador.procesar_archivo_banesco_planta(ruta)
    ...
    if not lista_dfs: 
        print("❌ No se obtuvieron datos válidos para Banesco Planta.")
        return False
    ...
    for (mes, moneda), grupo_df in df_total_consolidado.groupby(['Mes', 'Moneda REF']):
        ...
        nombre_base = f"BanescoPlanta_{sufijo_moneda}_Mes_{mes}"
