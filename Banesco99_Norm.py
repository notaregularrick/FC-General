...
class NormalizadorBanesco99:
    ...
    def crear_carpetas(self):
        os.makedirs(f"{self.carpeta_salida}/banesco99", exist_ok=True)
    ...
    def limpiar_fecha_espanol(self, fecha_str):
        ...
        # 2. Limpiar posibles horas adjuntas para evitar confusiones
        s = s.split()[0]
        
        # 3. Detección inteligente del formato
        try:
            # Si empieza con 4 dígitos (ej: 2025/06/03 o 2025-06-03)
            if re.match(r'^\d{4}[-/]\d{1,2}[-/]\d{1,2}', s):
                return pd.to_datetime(s, errors='coerce', yearfirst=True)
            # Formato clásico donde el día va primero (ej: 03/06/2025 o 03-06-25)
            else:
                return pd.to_datetime(s, errors='coerce', dayfirst=True)
        except:
            return pd.NaT

    def procesar_archivo_banesco(self, archivo_path):
        nombre_archivo = os.path.basename(archivo_path)
        print(f"🔄 Leyendo archivo (Banesco 99): {nombre_archivo}")
        
        es_usd = any(x in nombre_archivo.lower() for x in ['usd', 'dolar', 'divisa', 'verde', 'panama'])
        ...
        try:
            ...
            for i in range(min(25, len(df_raw))): 
                ...
                if 'fecha' in fila_str and \
                   any(x in fila_str for x in ['monto', 'haber', 'debe', 'saldo', 'tipo', 'movimiento', 'transaccion']):
                    fila_inicio = i
                    break
            ...
            col_tipo = next((c for c in df.columns if any(x in c.lower() for x in ['tipo', 'movimiento', 'operación', 'operacion', 'transaccion'])), None)
            if col_tipo: print(f"   🐛 Columna 'Tipo' identificada como: '{col_tipo}'")
            ...
            for idx, fila in df.iterrows():
                try:
                    ...
                    # 🚀 Usamos la nueva función traductora de fechas
                    ...
                    texto_tipo = ""
                    if col_tipo:
                        texto_tipo = str(fila.get(col_tipo, '')).upper()
                    texto_desc = str(fila.get(col_desc, '')).upper()

                    keys_credito = ['CREDITO', 'CRÉDITO', 'DEPOSITO', 'DEPÓSITO', 'ABONO']
                    keys_debito = ['DEBITO', 'DÉBITO', 'RETIRO', 'CARGO', 'COMISION', 'COMISIÓN']

                    if any(k in texto_tipo for k in keys_credito): es_credito = True
                    elif any(k in texto_tipo for k in keys_debito): es_debito = True
                    
                    if not es_credito and not es_debito:
                        if any(k in texto_desc for k in ['NOTA DE CREDITO', 'NOTA DE CRÉDITO', 'DEPOSITO']): es_credito = True
                        elif any(k in texto_desc for k in ['NOTA DE DEBITO', 'NOTA DE DÉBITO', 'COMISION', 'COMISIÓN']): es_debito = True

                    if not es_credito and not es_debito:
                        fila_entera_str = " ".join([str(val).upper() for val in fila.values])
                        if 'COMISION' in fila_entera_str or 'COMISIÓN' in fila_entera_str or 'CARGO' in fila_entera_str:
                            es_debito = True
                    ...
                    if es_usd:
                        ...
                        banco_nombre = 'BANESCO_USD'    # Nombre con USD
                        tasa = 1.0
                        monto_usd = monto_bs
                    else:
                        moneda_ref = 'BS'               # Solo BS
                        banco_nombre = 'BANESCO'        # Nombre normal sin BS
                    ...
                    if texto_tipo and texto_tipo not in descripcion.upper():
                         descripcion = f"{texto_tipo} - {descripcion}"
    ...
    def guardar_archivo(self, df, nombre_base="Banesco"):
        ...
        if not fechas.empty:
            ...
            path = f"{self.carpeta_salida}/banesco99/{nombre_base} [{rango}].xlsx"
...
def ejecutar_normalizacion(scanner_global, gestor_tasas_global):
    print(f"\n🚀 --- Iniciando Módulo: BANESCO (CUENTA 99) ---")
    
    normalizador = NormalizadorBanesco99(
        gestor_tasas=gestor_tasas_global,
        carpeta_salida="estados_cuenta_processed"
    )

    archivos_encontrados = scanner_global.listar_archivos_simples(criterio_nombre="BANESCO")
    archivos_validos = [f for f in archivos_encontrados if not os.path.basename(f).startswith("~$")]

    if not archivos_validos:
        print("⚠️ No se encontraron archivos de Banesco.")
        return False
    ...
    for ruta in archivos_validos:
        ...
        # --- FILTROS DE EXCLUSIÓN ACTUALIZADOS ---
        if "PANAMA" in nombre_archivo:
            print(f"   ⚠️ Saltando archivo de PANAMÁ: {nombre_archivo}")
            continue
            
        if "PLANTA" in nombre_archivo:
            print(f"   ⚠️ Saltando archivo de PLANTA: {nombre_archivo}")
            continue

        if "VERDE" in nombre_archivo:
            print(f"   ⚠️ Saltando archivo de VERDE: {nombre_archivo}")
            continue
        # -----------------------------

        df_temp = normalizador.procesar_archivo_banesco(ruta)
    ...
    if not lista_dfs: 
        print("❌ No se obtuvieron datos válidos (Se filtraron los archivos de otras cuentas o estaban vacíos).")
        return False
    ...
    for (mes, moneda), grupo_df in df_total_consolidado.groupby(['Mes', 'Moneda REF']):
        sufijo = "USD" if moneda == 'US' else "BS"
        nombre_base = f"Banesco99_{sufijo}_Mes_{mes}"
