...
class NormalizadorBNC6550:
    ...
    def crear_carpetas(self):
        # Carpeta independiente para la cuenta 6550
        os.makedirs(f"{self.carpeta_salida}/bnc_6550", exist_ok=True)
    ...
    def procesar_archivo_bnc(self, archivo_path):
        ...
        print(f"🔄 Leyendo archivo (BNC 6550): {nombre_archivo}")
        ...
        try:
            ...
            # Limpiar saltos de línea molestos en los encabezados
            ...
            for idx, fila in df.iterrows():
                try:
                    ...
                    if col_cargo and col_abono:
                        ...
                        if abs(cargo) > 0:
                            val_absoluto = abs(cargo)
                            tipo_op = "DEBITO"
                            factor = -1.0
                        elif abs(abono) > 0:
                            val_absoluto = abs(abono)
                            tipo_op = "CREDITO"
                            factor = 1.0
                    elif col_monto:
                    ...
                    if es_dolares:
                        moneda_ref = 'US'        
                        banco_nombre = 'BNC6550_USD' 
                        tasa = 1.0
                        monto_usd = monto_bs
                    else:
                        moneda_ref = 'BS'        
                        banco_nombre = 'BNC6550'     
    ...
    def guardar_archivo(self, df, nombre_base="BNC6550"):
        ...
        if not fechas.empty:
            ...
            path = f"{self.carpeta_salida}/bnc_6550/{nombre_base} [{rango}].xlsx"
...
def ejecutar_normalizacion(scanner_global, gestor_tasas_global):
    print(f"\n🚀 --- Iniciando Módulo: BNC 6550 ---")
    
    normalizador = NormalizadorBNC6550(
    ...
    for ruta in archivos_validos:
        ...
        # --- FILTRO INVERTIDO ---
        # Si el archivo NO contiene "6550", lo saltamos
        if "6550" not in os.path.basename(ruta):
            print(f"   ⚠️ Saltando archivo BNC normal (No es 6550): {os.path.basename(ruta)}")
            continue
        # ----------------------
    ...
    if not lista_dfs: 
        print("❌ No se obtuvieron datos válidos para BNC 6550.")
        return False
    ...
    for (mes, moneda), grupo_df in df_total_consolidado.groupby(['Mes', 'Moneda REF']):
        nombre_base = f"BNC6550_{moneda}_Mes_{mes}"
