...
class NormalizadorBDV:
    ...
    def crear_carpetas(self):
        os.makedirs(f"{self.carpeta_salida}/bdv", exist_ok=True)
    ...
    def procesar_archivo_bdv(self, archivo_path):
        nombre_archivo = os.path.basename(archivo_path)
        print(f"🔄 Leyendo archivo (BDV): {nombre_archivo}")
        ...
        try:
            ...
            for i in range(min(25, len(df_raw))): 
                ...
                if 'fecha' in fila_str and \
                   any(x in fila_str for x in ['monto', 'cargo', 'abono', 'saldo', 'debito', 'credito']):
                    fila_inicio = i
                    break
            ...
            if not col_ref: col_ref = next((c for c in df.columns if any(x in c.lower() for x in ['documento', 'doc', 'nro'])), None)

            col_cargo = next((c for c in df.columns if any(x in c.lower() for x in ['cargo', 'retiro', 'debito', 'débito'])), None)
            col_abono = next((c for c in df.columns if any(x in c.lower() for x in ['abono', 'deposito', 'credito', 'crédito'])), None)
            ...
            for idx, fila in df.iterrows():
                try:
                    ...
                    if es_usd:
                        moneda_ref = 'US'               # Nomenclatura US
                        banco_nombre = 'BDV_USD'        # Nombre con USD
                        tasa = 1.0
                        monto_usd = monto_bs
                    else:
                        moneda_ref = 'BS'               # Nomenclatura BS
                        banco_nombre = 'BDV'            # Nombre normal
    ...
    def guardar_archivo(self, df, nombre_base="BDV"):
        ...
        if not fechas.empty:
            ...
            path = f"{self.carpeta_salida}/bdv/{nombre_base} [{rango}].xlsx"
...
def ejecutar_normalizacion(scanner_global, gestor_tasas_global):
    print(f"\n🚀 --- Iniciando Módulo: BDV ---")
    
    normalizador = NormalizadorBDV(
        gestor_tasas=gestor_tasas_global,
        carpeta_salida="estados_cuenta_processed"
    )

    # Buscar archivos "BDV" o "VENEZUELA"
    archivos_encontrados = scanner_global.listar_archivos_simples(criterio_nombre="BDV")
    if not archivos_encontrados:
        archivos_encontrados = scanner_global.listar_archivos_simples(criterio_nombre="VENEZUELA")
        
    archivos_validos = [f for f in archivos_encontrados if not os.path.basename(f).startswith("~$")]

    if not archivos_validos:
        print("⚠️ No se encontraron archivos de Banco de Venezuela.")
        return False

    lista_dfs = []
    for ruta in archivos_validos:
        df_temp = normalizador.procesar_archivo_bdv(ruta)
    ...
    if not lista_dfs: 
        print("❌ No se obtuvieron datos válidos para BDV.")
        return False
    ...
    for (mes, moneda), grupo_df in df_total_consolidado.groupby(['Mes', 'Moneda REF']):
        ...
        nombre_base = f"BDV_{sufijo_moneda}_Mes_{mes}"
