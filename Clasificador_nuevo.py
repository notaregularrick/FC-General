...
def Clasificarbalance(
    ...
    res[texto_norm_col] = res[texto_col].astype(str).map(_normalize_text)
    res[texto_tokens_col] = res[texto_norm_col].map(_tokenize)

    if verbose:
        longitudes = res[texto_tokens_col].map(len)
        print(
            f"   - Registros con tokens vacíos: {(longitudes == 0).sum()} "
            f"de {len(res)}"
        )
        print(
            f"   - Tokens promedio por registro (solo >0): "
            f"{longitudes[longitudes > 0].mean():.2f}"
        )
    ...
    while umbral_actual >= umbral_minimo and iteracion < max_iteraciones:
        ...
        for idx_pos, idx in enumerate(indices_pendientes, start=1):
            a_norm = res.at[idx, texto_norm_col]
            a_toks = res.at[idx, texto_tokens_col]
            ...
            if verbose and (idx_pos % 500 == 0 or idx_pos == total_iter):
                pendientes_post = (res["clasificacion"] == "Sin clasificacion").sum()
                print(
    ...
    res = res.drop(columns=[texto_norm_col, texto_tokens_col])
